...
if ( !class_exists('aspkImportVehicles')){
	class aspkImportVehicles{
		...
		function addCronMinutes($array) {
			...
			'interval' => 3600, 
			'display' => __( 'Every Fifteen Minutes' , 'texthomain' )
			);
			...
			/* $schedules['every_three_minutes'] = array(
				'interval'  => 43200,
				'display'   => __( 'Every 3 Minutes', 'texthomain' )
			); */
			return $schedules;
		}
		...
		function install () {
			if ( ! wp_next_scheduled( 'my_task_hook' ) ) {
				...
				wp_schedule_event( time(), 'fifteen_minutes', 'my_task_hook');
			}
		}
		...
		function importVehiclesTabular(){
			$files  =   glob(ABSPATH . "/QUnitUpload_9IO8CJI4FDBU24Z42HR9_AllUnits.csv");
			if($files[0]){
				$file_path = $files[0];
		...
		function importVehicles(){
				...
				<div class="row">
					<div class="col-md-6 justify-center">
						<form method="post" action="" enctype="multipart/form-data">
							...
							<div class="row">
								<input type="submit" name="aspk_import_file" class="btn btn-primary" value="Import Vehicle Data">
							</div>
						</form>
					</div>
				</div>
			...
			if(isset($_POST["aspk_import_file"])){*/
				...
				// 	$this->importVehiclesDetail($filepath);
				// }
			//}
		}
		...
		function manageVINCurl($vin, $post_id){
			...
			if ($err) {
				...
				$engineType = $vehicle_array['engineType'];
				$driveType = $vehicle_array['driveType'];
				$fuelType = $vehicle_array['fuelType'];*/
